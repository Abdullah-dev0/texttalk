...
const ChatComponent = ({ file }: SimpleChatProps) => {
	...
	// Use the AI SDK useChat hook with proper transport
			...
			<div className="flex-1 overflow-y-auto overflow-x-hidden p-4 space-y-4 min-h-0">
				{messages.length === 0 ? (
					<div className="flex flex-col items-center justify-center h-full text-center max-w-md mx-auto">
						...
						<p className="text-zinc-500">Ask a question about your PDF in {selectedLanguage}.</p>
					</div>
				) : (
							...
							<div
								...
								<div className="max-w-[80%] p-3 rounded-lg bg-white border shadow-sm break-words">
									{message.parts.map((part, index) =>
										part.type === "text" ? (
											<div key={index}>
												<ReactMarkdown className="prose prose-sm max-w-none">{part.text}</ReactMarkdown>
											</div>
										) : null,
									)}
								</div>
			...
			<div className="border-t bg-white p-4 flex-shrink-0">
				<div className="max-w-4xl mx-auto">
					<form onSubmit={handleSubmit} className="flex gap-2 items-end">
						...
						/>
						<Button type="submit" disabled={status !== "ready" || !input.trim()} className="flex-shrink-0">
